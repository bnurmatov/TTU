				...
				case Element.ANNOTATION:
					...
					if (line != null)
						rect = new RectangleElement( 
							...
							annot.getUry(indentTop - currentHeight - 0), 
					...
					if( chunk.isStroked() )
					{
						...
						if( chunk.isAttribute( Chunk.UNDERLINE ) )
						{
							...
							for( k = 0; k < unders.length; ++k )
							{
								...
								var lthickness: Number =  ps[0] + fsize * ps[1];
								var shift: Number = ps[2] + fsize * ps[3];
								var cap2: int = ps[4];
								
								var ttu:TrueTypeFont = chunk.font && chunk.font.font? chunk.font.font as TrueTypeFont : null;
								if(ttu)
								{
									lthickness = ps[1];
									cap2 	   = ps[4];
								...
								graphics.setLineWidth( lthickness );
