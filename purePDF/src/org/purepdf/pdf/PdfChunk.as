...
package org.purepdf.pdf
{
	...
	public class PdfChunk extends ObjectHash
	{
		...
		internal function split( width: Number ): PdfChunk
		{
			...
			if ( hyphenationEvent != null && lastSpace >= 0 && lastSpace < currentPosition )
			{
				...
				if ( wordIdx > lastSpace )
				{
					...
					if ( pre.length > 0 )
					{
						...
						value = StringUtils.right_trim( value.substring( 0, lastSpace ) + pre );
						pc = PdfChunk.fromString( returnValue, this );
						return pc;
					}
				}
			...
			value = StringUtils.right_trim(value.substring( 0, splitPosition ) );
			pc = PdfChunk.fromString( returnValue, this );
			return pc;
		}
		...
		public static function fromChunk( chunk: Chunk, action: PdfAction ): PdfChunk
		{
			...
			if ( f.isUnderline )
			{
				var ttu:TrueTypeFont = f.baseFont as TrueTypeFont;
				tmp = new Vector.<Object>(2);
				if(ttu)
				{
					var lthickness: Number	= ttu.getFontDescriptor(BaseFont.UNDERLINE_THICKNESS, f.size);
					var shift: Number 		= ttu.getFontDescriptor(BaseFont.UNDERLINE_POSITION, f.size);
					var cap2: int 			= ttu.getFontDescriptor(BaseFont.CAPHEIGHT, f.size);
					tmp[1] 					= Vector.<Number>( [ 0, lthickness, 0, shift, cap2 ] );
				}
				else
