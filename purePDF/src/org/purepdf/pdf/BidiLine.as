...
package org.purepdf.pdf
{
	...
	public class BidiLine
	{
		...
		public function addPiece( c: int, chunk: PdfChunk ): void
		{
			...
			if(totalTextLength >= text.length)
			{
				text.fixed = false;
				while ( totalTextLength >  text.length - 1)
					text.push(new PdfChunk());
				text.fixed = true;
			}
		...
		public function restore(): void
		{
			...
			if ( runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL )
			{
/*				orderLevels 		= storedOrderLevels.slice( currentChar );
				orderLevels.length 	= totalTextLength - currentChar;
				indexChars 			= storedIndexChars.slice( currentChar );
				indexChars.length 	= totalTextLength - currentChar;*/
			}
		}
		...
		public function save(): void
		{
			...
			if ( runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL )
			{
				if (storedOrderLevels.length < totalTextLength) {
					storedOrderLevels = new Vector.<int>(currentChar);
					storedIndexChars = new Vector.<int>(currentChar);
				}
				var tempOrderLevels:Vector.<int> = orderLevels.slice(currentChar);
				var tempIndexChars:Vector.<int> = indexChars.slice(currentChar);
				storedOrderLevels.length = currentChar;
				storedIndexChars.length = currentChar;
				storedOrderLevels =  storedOrderLevels.concat(tempOrderLevels);
				storedIndexChars =  storedIndexChars.concat(tempIndexChars);
