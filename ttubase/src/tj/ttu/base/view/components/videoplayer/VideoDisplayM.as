...
package tj.ttu.base.view.components.videoplayer
{
	 ...
	 *  @productversion Flex 4
	 */
	public class VideoDisplayM extends UIComponent
	{
		...
		/**
		 * @private
		 * seek to this time after player be ready.
		 */
		private var seeking:Number = 0;
		//--------------------------------------------------------------------------
		//
		//  Properties
		//
		...
		public function get muted():Boolean
		{
			...
			if (inLoadingState2 || inLoadingState1)
		 ...
		 *  video player.
		 */
		public function videoPlayerResponsive():Boolean
		{
		...
		private function createUnderlyingVideoPlayer():void
		{
			...
			// internal events
			videoPlayer.addEventListener(DisplayObjectEvent.MEDIA_SIZE_CHANGE, videoPlayer_mediaSizeChangeHandler, false, 0, true);
			videoPlayer.addEventListener(AudioEvent.VOLUME_CHANGE, videoPlayer_volumeChangeHandler, false, 0, true);
			videoPlayer.addEventListener(AudioEvent.MUTED_CHANGE, videoPlayer_mutedChangeHandler, false, 0, true);
			
			// public events
			videoPlayer.addEventListener(MediaPlayerStateChangeEvent.MEDIA_PLAYER_STATE_CHANGE, videoPlayer_mediaPlayerStateChangeHandler, false, 0, true);
			videoPlayer.addEventListener(TimeEvent.CURRENT_TIME_CHANGE, dispatchEvent, false, 0, true);
			videoPlayer.addEventListener(LoadEvent.BYTES_LOADED_CHANGE, dispatchEvent, false, 0, true);
			videoPlayer.addEventListener(TimeEvent.DURATION_CHANGE, videoPlayer_durationChangeHandler, false, 0, true);
			videoPlayer.addEventListener(TimeEvent.COMPLETE, dispatchEvent, false, 0, true);
		...
		private function load():void
		{
			...
			videoPlayer.addEventListener(MediaPlayerStateChangeEvent.MEDIA_PLAYER_STATE_CHANGE, videoPlayer_mediaPlayerStateChangeHandlerForLoading, false, 0, true);
		}
...
//			videoPlayer.media = null;
		...
		private function videoPlayer_mediaSizeChangeHandler(event:DisplayObjectEvent):void
		{
			...
			if (inLoadingState2)
			{
				...
				else if (duration < 0)
				{
					...
					videoPlayer.addEventListener(TimeEvent.CURRENT_TIME_CHANGE, videoPlayer_currentTimeChangeHandler, false, 0, true);
				}
				else
				{
					...
					videoPlayer.addEventListener(MediaPlayerCapabilityChangeEvent.CAN_SEEK_CHANGE, videoPlayer_canSeekChangeHandler, false, 0, true);
				}
			}
		}
		...
		private function pauseAndSeekCallBack():void
		{
			...
			if (videoPlayer)
			{
				videoPlayer.addEventListener(SeekEvent.SEEKING_CHANGE, videoPlayer_seekChangeHandler, false, 0, true);
				
				var videoPlayerMedia:org.osmf.media.MediaElement = videoPlayer.media;
				
				// called play(), now call pause() and seek(0);
				if(videoPlayerMedia && videoPlayerMedia.hasTrait(MediaTraitType.PLAY))
					videoPlayer.pause()
				
				if (videoPlayerMedia && videoPlayerMedia.hasTrait(MediaTraitType.SEEK))
				{
					seeking = isNaN(seeking) ? 0 : seeking;
					videoPlayer.seek(seeking);
					seeking = 0;				
				}
			}
		}
