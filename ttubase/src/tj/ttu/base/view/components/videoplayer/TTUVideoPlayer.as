...
package tj.ttu.base.view.components.videoplayer
{
	...
	*/	
	...
	/**
	 *  Replay state of the VideoPlayer
	 */
	[SkinState("replay")]
	[SkinState("replayAndFullScreen")]
	...
	/**
	 * TTUVideoPlayer class 
	 */
	public class TTUVideoPlayer extends SkinnableComponent
	{
		 ...
		 *  @productversion Flex 4
		 */
		public function TTUVideoPlayer()
		{
			super();
		}
		 ...
		 *  @productversion Flex 4
		 */
		public var videoDisplay:VideoDisplayM;
		 ...
		 *  @productversion Flex 4
		 */
		public var ppButton:ToggleButtonBase;
		...
		{
			if (TTUVideoPlayer.createAccessibilityImplementation != null)
				TTUVideoPlayer.createAccessibilityImplementation(this);
		}
		...
		{   
			var state:String = (videoDisplay && videoDisplay.videoPlayer)
				? videoDisplay.videoPlayer.state : "disabled";
		...
		override protected function partAdded(partName:String, instance:Object):void
		{
			...
			if (instance == videoDisplay)
			{
				...
				videoDisplay.addEventListener(TimeEvent.COMPLETE, handlerComplete);
			...
			else if (instance == ppButton)
			{
				ppButton.addEventListener(MouseEvent.CLICK, playPauseButton_clickHandler);
			}
			else if (instance == replayButton)
			{
				replayButton.addEventListener(MouseEvent.CLICK, replayButton_clickHandler);
			}
			else if (instance == buttonClose)
			{
				buttonClose.addEventListener(MouseEvent.CLICK, buttonClose_clickHandler);
			}
		...
		override protected function partRemoved(partName:String, instance:Object):void
		{
			...
			{
				ppButton.removeEventListener(MouseEvent.CLICK, playPauseButton_clickHandler);
			}
			else if (instance == replayButton)
			{
				replayButton.removeEventListener(MouseEvent.CLICK, replayButton_clickHandler);
			}
			else if (instance == buttonClose)
			{
				buttonClose.removeEventListener(MouseEvent.CLICK, buttonClose_clickHandler);
			}
			else if (instance == stopButton)
			{
		...
		private function videoDisplay_currentTimeChangeHandler(event:TimeEvent):void
		{
			...
			isReplay = false;// AZH: Replay State
		...
		private function playPauseButton_clickHandler(event:MouseEvent):void
		{
			...
			if(playPauseButton)
				playPauseButton.selected = playing;
			if(ppButton)
				ppButton.selected = playing;
		}
		
		private function replayButton_clickHandler(event:MouseEvent):void
		{
			stop();
			isReplay = false;
			invalidateSkinState();
		...
		private function buttonClose_clickHandler(event:MouseEvent):void
		{
			stop();
		...
		{
			seek(scrubBar.value);
			dispatchEvent(new TimeEvent("scrubBarChange", false, false,scrubBar.value));
		}
		...
		private function handlerComplete(event:TimeEvent):void
		{
			dispatchEvent(event);
